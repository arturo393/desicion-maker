...
<header class="header"><h1>Decision Intelligence Report</h1><div><span style="color:#8b949e;margin-right:1rem">{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</span><span class="badge">{mode.upper()} TIER</span></div></header>
...
<div class="kpi-card"><div class="kpi-title">Pareto Efficient</div><div class="kpi-value">{pareto_count} Options</div><div class="kpi-sub">Non-dominated frontier</div></div>"""
    ...
    for name, data in algo_comp.items():
        ...
        top_r = f'<span style="color:#3fb950;font-weight:bold">{top_r}</span>' if data.get("topsis_rank") == 1 else top_r
        html += f"<tr><td>{name}</td><td>{mc_r}</td><td>{top_r}</td>"
        if has_prom:
            prom_r = f"#{data.get('promethee_rank', '-')}"
            prom_r = f'<span style="color:#3fb950;font-weight:bold">{prom_r}</span>' if data.get("promethee_rank") == 1 else prom_r
            html += f"<td>{prom_r}</td>"
        html += "</tr>"
    html += "</tbody></table></div>"
    ...
    html += '<div class="panel col-12"><h2 class="panel-title">Risk Profiles (95% Confidence)</h2><div class="stats-grid">'
    for name, stats in mc_results.items():
