...
class TestPaprikaWeights:
    def test_basic_paprika(self):
        ...
        result = WeightDerivationEngine.paprika_weights(
            ["Cost", "Quality", "Speed"], answers
        )
        assert "weights" in result
        assert abs(sum(result["weights"].values()) - 1.0) < 1e-6
...
class TestDeriveConvenience:
    ...
    def test_derive_pairwise(self):
        result = WeightDerivationEngine.derive(
            "pairwise", comparisons={("A", "B"): 3}, labels=["A", "B"]
        )
        assert "weights" in result

    def test_derive_paprika(self):
        result = WeightDerivationEngine.derive(
            "paprika", factors=["A", "B"], tradeoff_answers={("A", "B"): "A"}
        )
        assert "weights" in result
