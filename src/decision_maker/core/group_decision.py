"""
Group decision making consensus aggregator across multiple stakeholder preferences.
Usage: from decision_maker.core.group_decision import GroupDecisionEngine
Does NOT: Calculate single-stakeholder Monte Carlo simulations.
"""
...
class GroupDecisionEngine:
    ...
    def aggregate_weights(
        ...
        if method == "median":
            ...
            log_matrix = np.log(np.maximum(matrix, 1e-9))
            geo_mean = np.exp(np.mean(log_matrix, axis=0))
            consensus = geo_mean / geo_mean.sum()
        elif method == "borda":
        ...
        return {
            ...
            "consensus_level": ("high" if kendall_w > 0.7 else "moderate" if kendall_w > 0.4 else "low"),
    ...
    def aggregate_scores(
        ...
        result["factors"] = [
            Factor(name=f.name, weight=consensus.get(f.name, f.weight), maximize=f.maximize, category=f.category)
            for f in factors
        ]
        return result
