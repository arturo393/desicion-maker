...
def two_options():
    ...
    return {
        "SafeA": Statistics(
            "SafeA", 0.7, 0.1, 0.4, 0.95, 0.53, 0.87, 0.95, {"X": {"mean": 10}}, 0.53, 0.48, raw_scores=a_scores
        ),
        "RiskyB": Statistics(
            "RiskyB", 0.5, 0.2, 0.0, 0.98, 0.17, 0.83, 0.6, {"X": {"mean": 10}}, 0.17, 0.12, raw_scores=b_scores
        ),
    }
...
class TestPortfolioOptimizer:
    ...
    def test_optimize_single_option(self):
        stats = Statistics("Only", 0.5, 0.1, 0.3, 0.7, 0.4, 0.6, 0.8, {"X": {"mean": 5}}, 0.4, 0.35)
    ...
    def test_three_options(self):
        ...
        for name, mean, std in [("A", 0.8, 0.1), ("B", 0.6, 0.15), ("C", 0.4, 0.25)]:
            ...
            res[name] = Statistics(name, mean, std, 0, 1, 0, 1, 0.5, {"X": {"mean": 5}}, 0, 0, raw_scores=scores)
