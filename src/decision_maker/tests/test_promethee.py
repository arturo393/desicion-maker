...
class TestPrometheeEngine:
    def test_basic_ranking(self):
        ...
        scores = engine.analyze(df, [0.5, 0.5], [False, True])
        assert len(scores) == 3

    def test_single_option(self):
        df = pd.DataFrame({"Cost": [100]}, index=["Only"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [False])
        assert len(scores) == 1
    ...
    def test_no_columns_returns_empty(self):
        ...
        engine = PrometheeEngine()
        scores = engine.analyze(df, [], [])
        assert scores.empty

    def test_two_options(self):
        df = pd.DataFrame({"X": [10, 20]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True])
    ...
    def test_all_weights_zero(self):
        df = pd.DataFrame({"X": [10, 20]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [0.0, 0.0], [True, True])
        assert scores["A"] == scores["B"] == 0.0

    def test_pref_type_usual(self):
        df = pd.DataFrame({"X": [10, 20]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True], pref_types=["usual"])
        assert scores.index[0] == "B"

    def test_pref_type_ushape_with_q(self):
        df = pd.DataFrame({"X": [10, 15]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores_no_pref = engine.analyze(df, [1.0], [True], pref_types=["ushape"], pref_params=[{"q": 10}])
        scores_pref = engine.analyze(df, [1.0], [True], pref_types=["ushape"], pref_params=[{"q": 3}])
    ...
    def test_pref_type_vshape(self):
        ...
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True], pref_types=["vshape"], pref_params=[{"p": 20}])
        assert 0 < scores["B"] < 1.0

    def test_pref_type_linear(self):
        df = pd.DataFrame({"X": [0, 10]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True], pref_types=["linear"], pref_params=[{"q": 2, "p": 20}])
        assert 0 < scores["B"] < 1.0

    def test_pref_type_level(self):
        df = pd.DataFrame({"X": [0, 10]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True], pref_types=["level"], pref_params=[{"q": 5, "p": 15}])
        assert scores["B"] == 0.5

    def test_pref_type_gaussian(self):
        df = pd.DataFrame({"X": [0, 3]}, index=["A", "B"])
        engine = PrometheeEngine()
        scores = engine.analyze(df, [1.0], [True], pref_types=["gaussian"], pref_params=[{"s": 2}])
        assert 0 < scores["B"] < 1.0
